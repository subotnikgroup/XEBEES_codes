...
python 2D/exact_2D.py -g_1 1 -g_2 1 -M_1 300 -M_2 300 -k 5 -R 21 -r 22 -g 24 --verbosity 10 --preconditioner BO
```
...
Available parameters vary by solver. For complete options, run:

```bash
python 2D/exact_2D.py --help
python 2D/ps_2D.py --help
python 3D/exact_3D.py --help
python 3D/ps_3D.py --help
```
...
python 2D/exact_2D.py -k 10 -g_1 1.0 -g_2 1.0 -M_1 100 -M_2 100 -r 50 -g 50 -R 50  -t 1 --verbosity 9 --preconditioner "BO" --backend cupy &> log_test
```
...
│   ├── exact_2D.py                     # Exact 2D solver
│   ├── ps_2D.py                        # Phase-space solver
│   ├── notebook-2D.ipynb               # examples and tutorials
├── 3D/     # Three-dimensional calculations
│   ├── exact_3D.py                     # Exact 3D solver
│   ├── ps_3D.py                        # Phase-Space 3D solver
│   ├── DBOC_3D.py                      # DBOC correction calculations
...
This project is licensed under the MIT License 2.0. See `LICENSE` for details.

`lib/linalg_helper.py` includes code derived from Apache License 2.0 material and retains its original license notice.
